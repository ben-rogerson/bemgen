...
import { bemChildTypePrefix, light } from '../utilities'
...
const ExportItem = props => {
  ...
  const
    { type, item, listItems, blockName, index, config, isSelectorNested } = props,
    prevItem = typeof listItems[index-1] !== 'undefined' ? listItems[index-1] : [],
    nextItem = typeof listItems[index+1] !== 'undefined' ? listItems[index+1] : [],
    isNextItemModifier = nextItem.typeId === 1 || false
  ...
  const nestedPrefix = '&' + prefix,
    nonNestedPrefix = '.' + nonNestedPrefixStart + prefix
